...
function initializeRootNode(scanViewer) {
    return scanViewer.createTileForGeometry(0, 0, 0).then(function (rootTile) {
        ...
        geometryRoot.setMatrix(
            osg.mat4.fromTranslation(osg.mat4.create(), osg.vec3.fromValues(-w / 2, h / 2, 0))
        );
        ...
        var manipulator = new PlanarOrbitManipulator({
            inputManager: scanViewer.viewer.getInputManager()
        });
        ...
        var screenSizeInPixels =
            0.5 *
            (scanViewer.viewer.getCanvasClientWidth() + scanViewer.viewer.getCanvasClientHeight());
...
var ScanViewer = function (canvasElement, options) {
    ...
    if (defined(textureMapTileSource)) {
        ...
        promises.push(
            textureMapTileSource.initializationPromise.then(function () {
    ...
    if (defined(normalMapTileSource)) {
        ...
        promises.push(
            normalMapTileSource.initializationPromise.then(function () {
    ...
    if (defined(glossMapTileSource)) {
        ...
        promises.push(
            glossMapTileSource.initializationPromise.then(function () {
    ...
    if (defined(elevationTileSource)) {
        ...
        promises.push(
            elevationTileSource.initializationPromise.then(function () {
    ...
    if (defined(options.Shading) && defined(options.Shading.LightSources)) {
        ...
        options.Shading.LightSources.forEach(function (light) {
            that.setLightParameters(
                index,
                options.Shading.Ambient || 0.2,
                light.Diffuse || 1.0,
                light.Specular || 0.1,
                options.Shading.PhongExponent || 10
            );
            index++;
        });
    }
...
ScanViewer.prototype = {
    ...
    fetchAndApplyDisplacementMap: function (x, y, level, stateSet) {
        ...
        return this._input
            ...
            .then(function (img) {
                ...
                texture.setTextureOffsetAndScale(
                    osg.vec4.fromValues(1.0 / e.w, 1.0 / e.h, 1.0 - 2.0 / e.w, 1.0 - 2.0 / e.h)
                );
    ...
    fetchAndApplyAllTileImagery: function (x, y, level, node, parentGeometry) {
        ...
        if (this._renderTextureMaps) {
            if (this._textureMapTileSource.hasTile(x, y, level)) {
                promises.push(
                    this.fetchAndApplyTileImagery(
                        x,
                        y,
                        level,
                        stateSet,
                        0,
                        this._textureMapTileSource
                    )
                );
            } else {
                ...
                var parentTileDomainTransformAttribute =
                    parentStateSet.getAttribute('TileDomainTransform');

                var offsetScale = osg.vec4.create();
                if (defined(parentTileDomainTransformAttribute)) {
                    offsetScale = osg.vec4.clone(
                        parentTileDomainTransformAttribute.getTextureOffsetAndScale()
                    );
                } else {
        ...
        if (this._renderNormalMaps) {
            if (this._normalMapTileSource.hasTile(x, y, level)) {
                ...
                var textureUnit = 1;
                var parentTexture = parentStateSet.getTextureAttribute(
                    textureUnit,
                    'NormalTexture'
                );
                stateSet.setTextureAttributeAndModes(textureUnit, parentTexture);
    ...
    _getOrCreateLight: function (lightIndex) {
        console.assert(
            lightIndex <= this._light.length,
            'Light source array must be populated consecutively.'
        );
        if (lightIndex > this._light.length) {
            return undefined;
        }
    ...
    _hasChildren: function (x, y, level) {
        return (
            (this._elevationTileSource && this._elevationTileSource.hasChildren(x, y, level)) ||
    ...
    createTileForGeometry: function (x, y, level, parentNode) {
            ...
            for (var i = 0; i < 4; i++) {
                ...
                (function (child) {
                    if (that._hasTile(child.x, child.y, child.level)) {
                        childPromises.push(
                            that.createTileForGeometry(child.x, child.y, child.level, parentNode)
                        );
                    }
                })(addr);
            }
        ...
        return this.fetchAndApplyAllTileImagery(x, y, level, tileGeometry, parentTileGeometry).then(
            function () {
                var tile;
    ...
    transformSphericalToWorld: function (elevation, azimuth, distance) {
        var direction = osg.vec3.fromValues(
