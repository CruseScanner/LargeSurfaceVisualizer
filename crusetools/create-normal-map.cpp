...
int main(int argc, char** argv)
{
	if (argc != 4)
	{
		std::cout<<"Usage: create-normal-map <zscale> <elev.tif> <normal-map.tif>"<<std::endl;
		std::cout<<std::endl;
		std::cout<<"zscale is height scale divided by horizontal scale (pixel size)."<<std::endl;
		std::cout<<std::endl;
		return 1;
	}
	const float scale = atof(argv[1]);
	const std::string elevationFilename = argv[2];
	const std::string normalMapFilename = argv[3];
